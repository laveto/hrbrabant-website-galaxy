        ...
        >
            ...
            @canvasLoop(['items' =>  3])
                ...
                <div class="h-full item"
                     @canvasEditableAttrs(['editor' => 'loop-LoopItemEditor', 'label' => 'Item', 'itemPanel' => 'LoopItemPanel'])
                >

                    <div class="relative {{ $canvasBlock->options->rounded ?? '' }} overflow-hidden">
                        @canvasBlock('canvas::utils.image', [
                            'key' => 'card-icon',
                            'class' => implode(' ', ['object-contain', ($canvasBlock->options->aspect_ratio ?? 'aspect-[16/9]')]),
                        ...
                        <div class="absolute inset-0 z-10 flex items-center justify-center w-full h-full text-5xl text-white bg-black/50">
